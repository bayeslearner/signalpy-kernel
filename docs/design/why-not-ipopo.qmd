...
So it is a fair question why an agent runtime would not simply use it. Four
...
That is the same gap signalpy had, and it is the one thing plugkit exists to
close.
...
through it by convention — which is what `signalpy` did, and what left the gate
bypassable from any caller that did not use it.
