# `signalpy2/cordis` — vendored, with changes

## Where it came from

`src/signalpy2/cordis/` is a vendored copy of **[geohotstan/cordis-py]** (MIT), a pure-Python
...
(2026-08-14). Its test suite is vendored alongside, under `signalpy2/tests/`.
...
`signalpy2/tests/test_conformance.py`:
...
- `signalpy2/tests/` — 16 listeners updated to the Cordis signature
...
Imports are `signalpy2.cordis`, not top-level `cordis`, so 1.0 (`src/signalpy`) and 2.0 can be
